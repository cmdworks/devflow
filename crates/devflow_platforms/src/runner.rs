...
pub trait PlatformRunner: Send + Sync {
    async fn install(&self, project_dir: &Path, device: &Device, artifact_path: Option<&str>) -> Result<()>;
    ...
    async fn restart(&self, project_dir: &Path, device: &Device, launch_cmd: Option<&str>) -> Result<()> {
        self.stop(project_dir, device).await?;
        self.launch(project_dir, device, launch_cmd).await
    }
    async fn stream_logs(&self, project_dir: &Path, device: &Device, tx: mpsc::Sender<LogEntry>) -> Result<tokio::task::JoinHandle<()>>;
}
