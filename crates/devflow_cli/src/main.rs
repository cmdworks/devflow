...
#[derive(Parser, Debug)]
#[command(name = "devflow", author, version, about = "Universal, framework-aware development runner and MCP server", long_about = None)]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    // Initialize subscriber for non-stdio MCP modes
    let is_mcp_stdio = matches!(&cli.command, Some(Commands::Mcp { command: McpCommands::Serve { http: false, .. } }));
    if !is_mcp_stdio {
    ...
    if let Some(cmd) = cli.command {
        match cmd {
            Commands::Init { platform, framework } => handle_init(platform, framework).await,
            Commands::Doctor { project_path, json } => handle_doctor(project_path, json).await,
            Commands::Devices { boot, json } => handle_devices(boot, json).await,
            Commands::Dev { target, framework, no_tui, json } => handle_dev(target, framework, no_tui, json).await,
            Commands::Build { release, target, json } => handle_build(release, target, json).await,
            Commands::Logs { follow, level, tag, query, limit } => handle_logs(follow, level, tag, query, limit).await,
            Commands::Reload => handle_reload().await,
            Commands::Restart => handle_restart().await,
            Commands::Preview => handle_preview().await,
            Commands::Mcp { command } => match command {
                McpCommands::Serve { http, port, token } => handle_mcp_serve(http, port, token).await,
            },
        }
    } else {
        // Zero-arg: Launch interactive TUI Hub with workspace multi-target selector & cross-terminal sync
...
async fn handle_init(_platform: Option<String>, framework: Option<String>) -> anyhow::Result<()> {
    let current_dir = std::env::current_dir()?;
...
async fn handle_doctor(project_path: PathBuf, json: bool) -> anyhow::Result<()> {
...
async fn handle_devices(boot: Option<String>, json: bool) -> anyhow::Result<()> {
...
async fn handle_dev(target: Option<String>, framework: Option<String>, no_tui: bool, json: bool) -> anyhow::Result<()> {
    let current_dir = std::env::current_dir()?;
    ...
    if no_tui || json {
            ...
            loop {
                match event_rx.recv().await {
                    ...
                    Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                        // Channel dropped older events under high throughput; resume immediately
                    }
        ...
        // Keep running in headless mode until Ctrl+C
...
async fn handle_build(release: bool, target: Option<String>, json: bool) -> anyhow::Result<()> {
    let current_dir = std::env::current_dir()?;
...
async fn handle_logs(
    ...
    if follow {
        ...
        // One-off retrieval of recent logs
...
async fn handle_preview() -> anyhow::Result<()> {
    let current_dir = std::env::current_dir()?;
    TuiRunner::run_hub(current_dir).await.map_err(|e| anyhow::anyhow!("{}", e))
}

async fn handle_mcp_serve(http: bool, port: u16, token: Option<String>) -> anyhow::Result<()> {
...
async fn handle_hub_summary(current_dir: &std::path::Path) -> anyhow::Result<()> {
