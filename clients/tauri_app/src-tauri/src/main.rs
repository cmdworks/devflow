    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            let app = Router::new()
                ...
                .route("/api/updater/check", get(handle_updater_check))
                .route("/api/updater/install", post(handle_updater_install))
                .route("/api/updater/restart", post(handle_updater_restart))
...
async fn handle_updater_check() -> Json<UpdateCheckResponse> {
...
async fn handle_updater_install(
    ...
    let tmp_dir = std::env::temp_dir().join(format!("devflow-update-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis()));
...
async fn handle_updater_restart() -> Json<serde_json::Value> {
    tokio::spawn(async {
